...
class BucketData(object):
    ...
    def flush_out(self):                           
        ...
        rs = {
            'img':torch.FloatTensor(img).to(self.device, non_blocking=True),
            'tgt_input':torch.LongTensor(tgt_input).to(self.device, non_blocking=True),
            'tgt_output':torch.LongTensor(tgt_output).to(self.device, non_blocking=True),
            'tgt_padding_mask':torch.BoolTensor(tgt_padding_mask).to(self.device, non_blocking=True),
            'filenames': filenames
        }
